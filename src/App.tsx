...
import { Calendar, MapPin, Shirt, Sparkles, Mail, Shield, ChevronUp } from 'lucide-react';
...
export default function App() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = ['hero-section', 'maps-section', 'program-section', 'dress-code-section', 'rsvp-section'];
    ...
    <>
      ...
      {isEnvelopeOpened && (
        <motion.div 
          ...
          className="relative min-h-screen bg-[#FCFAF7] text-stone-800 font-sans selection:bg-sage-100 selection:text-sage-900 overflow-x-hidden antialiased"
        >
        {/* Background visual textures */}
        <div className="fixed inset-0 pointer-events-none z-0">
          {/* Subtle top and bottom vignettes */}
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top,_var(--tw-gradient-stops))] from-sage-500/[0.015] via-transparent to-transparent" />
      ...
      <header className="fixed top-0 inset-x-0 z-40 bg-[#FCFAF7]/90 backdrop-blur-md border-b border-stone-200/50 transition-all">
        <div className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          >
            <span className="text-sage-700 font-semibold">NYAWIRA</span>
            <span className="text-stone-400 font-sans text-xs italic">&amp;</span>
            <span className="text-sage-700 font-semibold">MAINAH</span>
          </button>
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded-full transition-all flex items-center gap-1.5 cursor-pointer hover:text-stone-950 ${
                    activeSection === item.id 
                      ? 'bg-sage-100 text-sage-900 font-bold border border-sage-300'
          ...
          <button
            ...
            className="md:hidden px-4 py-1.5 bg-sage-700 hover:bg-sage-800 text-white font-sans font-extrabold text-[10px] uppercase tracking-widest rounded-full transition-all cursor-pointer shadow-xs"
          >
            RSVP NOW
          </button>
        </div>
      ...
      <div className="hidden lg:flex fixed right-8 top-1/2 -translate-y-1/2 z-40 flex-col gap-5 items-center">
        {navItems.map((item) => (
          ...
          >
            {/* Label Tooltip hover */}
            <span className="absolute right-full mr-4 bg-white/95 border border-stone-200 px-2.5 py-1 rounded text-[10px] font-sans font-bold uppercase tracking-wider shadow-md opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity duration-200 text-sage-800">
              {item.label}
            </span>
            {/* Dot node */}
            <span className={`w-2.5 h-2.5 rounded-full border transition-all ${
              activeSection === item.id 
                ? 'bg-sage-700 border-sage-600 scale-125' 
      ...
      <AnimatePresence>
        {showScrollTop && (
          ...
          >
            <button
              onClick={() => scrollToSection('hero-section')}
              className="p-3 bg-white hover:bg-stone-50 border border-stone-200/80 text-sage-800 rounded-full shadow-lg active:scale-95 transition-all cursor-pointer"
              title="Scroll to Top"
            >
