...
export default function RSVPForm() {
  ...
  const handleAdminLogin = (e: React.FormEvent) => {
    e.preventDefault();
    if (adminPassword === 'nyawiraandmainah2026' || adminPassword === 'admin') {
      setIsAuthenticated(true);
      setError('');
    } else {
      setError('Invalid passcode. Try "nyawiraandmainah2026".');
    }
  };
  ...
  const exportToCSV = () => {
    ...
    link.setAttribute("download", "nyawira_mainah_wedding_rsvps.csv");
          ...
          {submitted ? (
            <div className="text-center py-8 animate-fade-in flex flex-col items-center">
              ...
              <p className="text-xs text-stone-600 mt-4 max-w-sm mx-auto font-sans leading-relaxed">
                {status === 'attending' 
                  ? "We are absolutely thrilled to celebrate this special milestone with you at ACK Emmanuel Church Kikuyu and Leilani Gardens on Wednesday, 19th August 2026!"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <button
                    ...
                    className={`p-3.5 border rounded-lg text-center cursor-pointer transition-all ${
                      status === 'attending'
                        ? 'border-[#738A6E] bg-[#738A6E]/20 text-[#42523E] font-semibold shadow-inner'
                  ...
                  >
                    ...
                    <span className="text-[10px] font-light italic">See you in Kikuyu!</span>
                  </button>
          ...
          {!isAuthenticated ? (
            <form onSubmit={handleAdminLogin} className="max-w-sm mx-auto space-y-4 py-6 font-sans">
              <p className="text-xs text-stone-500 text-center mb-4">
                ...
                <span className="italic font-semibold text-[#738A6E]">Hint: nyawiraandmainah2026</span>
              </p>
              {error && (
